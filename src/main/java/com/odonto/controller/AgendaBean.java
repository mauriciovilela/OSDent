...
//	@NotNull
	...
	private void pageLoad() {
		//PF('ajaxLoading').show();
		RequestContext.getCurrentInstance().execute("PF('ajaxLoading').show();");
	...
	private String getTituloAgenda(AgendaOUT itemAgenda) {
		...
		if (StringUtils.isNotBlank(itemAgenda.getDsProcedimento()) &&
			StringUtils.isNotBlank(itemAgenda.getDsDescricao())) {
			titulo.append(" (");
...
//		} else if (status == Constants.TbAgendaStatus.desistencia) {
//			mensagem = "Consulta com desistência";
